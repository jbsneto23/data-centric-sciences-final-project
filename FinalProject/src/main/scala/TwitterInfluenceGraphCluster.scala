...
object TwitterInfluenceGraphCluster {
  ...
  // Reads a string in CSV format and creates an User object                 
  ...
  // Reads a string in CSV format and creates a Tweet object
  ...
  def main(args: Array[String]) {
    ...
    def makeEdges(tweet: Tweet): List[Edge[String]] = {
      ...
      for (x <- 0 to (fields.length - 1)) {
        ...
        if (destin.isDefined)
          edges += Edge(origin.toLong, usersIdScreenNames.value.get(fields(x)).get, tweet.text)
      }
      return edges.toList
    }
    ...
    val default: User = User(0L, "John Doe", "", "", "", "", false, 0, 0, 0, 0, 0, "", 0, "", false, "", "")
    ...
    println("\nTop 10 influencers: ")
    top10Influencer.foreach(u => println(u._1 + " influences " + u._2))
